...
namespace WindowsFormsApp1
{
    class CommonUtil
    {
        string sIniPath = ".\\APTner_Agent.ini";
        string sDocPath = ".\\Doc\\";
        ...
        public void FileLogger(string log)
        {
            ...
            if (!File.Exists(sDocPath + "APTner_" + sDate + ".txt"))
            {
                using (StreamWriter sw = File.CreateText(sDocPath + "APTner_" + sDate + ".txt"))
                {
                }
            }

            using (StreamWriter sw = File.AppendText(sDocPath + "APTner_" + sDate + ".txt"))
            {
                sw.WriteLine(sLog);
            }
        }
